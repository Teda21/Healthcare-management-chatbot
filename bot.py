...
CSV_FILE = 'users.csv'
APPOINTMENTS_SCRIPT = 'appointment WITH CSV FILE.py'  # The script to open for appointments
...
def login():
    ...
    if authenticate(username, password):
        ...
        if username == "tewodros" and password == "1234":
            show_appointments()  # Navigate to the appointments script
        else:
            # Navigate to the conversation page (if applicable)
...
def show_conversation_page():
    ...
    # Open the "conversation.py" file
    subprocess.Popen(['python', 'conversation.py'])

# Function to navigate to the appointments script
def show_appointments():
    # Close the current window
    root.withdraw()

    # Open the appointments_with_csv_file.py script
    subprocess.Popen(['python', APPOINTMENTS_SCRIPT])
